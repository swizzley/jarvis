...
func TestDecomposeToPostData(t *testing.T) {
	...
	postDatums := DecomposeToPostData(myObj)
...
func TestDecomposeToPostDataAsJSON(t *testing.T) {
	...
	postDatums := DecomposeToPostDataAsJSON(myObj)
	...
	verify := []subType{}
	verifyErr := DeserializeJSON(&verify, postDatums[4].Value)
...
func TestPatchObject(t *testing.T) {
	...
	err := PatchObject(&myObj, patchData)
	assert.Nil(err)
